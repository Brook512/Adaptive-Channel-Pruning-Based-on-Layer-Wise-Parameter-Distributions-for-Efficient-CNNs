    ...
    def regularize(self, model, alpha=2**4, bias=False):
        for i, group in enumerate(self._groups):
            ...
            ####################
                    ...
                    g = w * _gamma.view( 1, -1, *([1]*(len(w.shape)-2))  ) 
                    layer.weight.grad.data[:, idxs]+=self.reg * g
...
if __name__ == "__main__":
    # Example usage
    weight = np.array([0.1, -0.05, 0.001, -0.001, 0.5])
    grad_original = np.array([0.02, -0.01, 0.005, -0.003, 0.04])
    epsilon = 0.01 # threshold阈值
    delta = 0.1 # 为一个调节参数，控制逻辑函数的平滑度
    lambda_ = 0.1 # 调节正则化梯度的占比
    eta = 0.5 # 调节最终总梯度的比例

    print("Initial weight:", weight)
    # weight = update_parameters(weight, grad_original, epsilon, delta, lambda_, eta)
